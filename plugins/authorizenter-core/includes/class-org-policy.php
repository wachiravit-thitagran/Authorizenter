...
class Org_Policy {
	...
	public function allowed_domains( $source = null ) {
		 ...
		 */
		return apply_filters( 'authorizenter_allowed_domains', array_values( array_unique( $domains ) ) );
	}
	...
	public function is_allowed( Identity $identity, $context = null ) {
		...
		if ( ! $enabled ) {
			...
			return apply_filters( 'authorizenter_is_allowed', true, $identity );
		}
		 ...
		 */
		return apply_filters( 'authorizenter_is_allowed', $result, $identity );
	}
	...
	public function check_capability( \WP_User $user, array $context ) {
		 ...
		 */
		$ok = apply_filters( 'authorizenter_context_capability', $ok, $user, $context );
		...
		return new \WP_Error(
			'authorizenter_insufficient_capability',
			__( 'Your account does not have permission to sign in here.', 'authorizenter' ),
			array(
				'status'   => 403,
				'reason'   => 'capability',
				'required' => $cap,
	...
	private function deny( $policy, $reason ) {
		$message = isset( $policy['block_message'] ) && '' !== $policy['block_message']
			...
			: __( 'Your account is not permitted to sign in to this site.', 'authorizenter' );
		return new \WP_Error(
			'authorizenter_denied',
			$message,
			array(
				'status' => 403,
				'reason' => $reason,
