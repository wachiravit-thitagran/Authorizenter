...
class OAuth_Engine {
	 ...
	 */
	const SESSION_KEY = 'authorizenter_oidc_flow';
	...
	public function redirect_uri() {
		return rest_url( AUTHORIZENTER_REST_NAMESPACE . '/callback' );
	}
	...
	public function begin( $provider_id, $return_to = '', $context_id = 'default', $extra_data = array() ) {
		...
		if ( ! $this->providers->is_allowed_in_context( $provider_id, $context ) ) {
			return new \WP_Error( 'authorizenter_provider_not_in_context', __( 'This sign-in method is not available here.', 'authorizenter' ), array( 'status' => 400 ) );
		}
	...
	public function handle_callback( $code, $state ) {
		...
		if ( '' === $code || '' === $state ) {
			return new \WP_Error( 'authorizenter_callback_missing', __( 'Missing authorization parameters.', 'authorizenter' ), array( 'status' => 400 ) );
		}

		$flow = get_transient( $this->flow_key( $state ) );
		if ( ! is_array( $flow ) ) {
			authorizenter_log( 'flow transient missing/expired for state' );
			return new \WP_Error( 'authorizenter_state_invalid', __( 'Login session expired or invalid. Please try again.', 'authorizenter' ), array( 'status' => 400 ) );
		}
		delete_transient( $this->flow_key( $state ) ); // single use.

		$provider = $this->providers->get( $flow['provider'] );
		if ( ! $provider || ! $provider->is_enabled() ) {
			authorizenter_log( 'provider disabled at callback', array( 'provider' => $flow['provider'] ) );
			return new \WP_Error( 'authorizenter_provider_disabled', __( 'This sign-in method is not available.', 'authorizenter' ), array( 'status' => 400 ) );
		}
	...
	private function handle_callback_oidc( array $session ) {
		...
		$provider    = $this->providers->get( $provider_id );
		if ( ! $provider || ! $provider->is_enabled() ) {
			authorizenter_log( 'provider disabled at callback', array( 'provider' => $provider_id ) );
			return new \WP_Error( 'authorizenter_provider_disabled', __( 'This sign-in method is not available.', 'authorizenter' ), array( 'status' => 400 ) );
		}
	...
	private function finish_login( Identity $identity, array $context, Provider_Base $provider, $return_to, $extra_data = array() ) {
		 ...
		 * @param array    $context  Resolved login context.
		 */
		$identity = apply_filters( 'authorizenter_identity', $identity, $context );
		...
		if ( ! headers_sent() ) {
			...
			authorizenter_log( 'auth cookie issued', array( 'logged_in_cookie_present' => ! empty( $set_cookie ) ) );
		}
	 ...
	 * `authorizenter_sso_logout`) and the user's last provider supports RP-initiated
	...
	public function logout( $return_to = '' ) {
		...
		$provider_id = $user_id ? (string) get_user_meta( $user_id, 'authorizenter_last_provider', true ) : '';
		...
		if ( '' !== $provider_id ) {
			 ...
			 * @param string $provider_id Provider id.
			 */
			$sso = apply_filters( 'authorizenter_sso_logout', false, $provider_id );
			if ( $sso ) {
				$provider = $this->providers->get( $provider_id );
	...
	private function flow_key( $state ) {
		return 'authorizenter_flow_' . hash( 'sha256', $state );
	}
