 ...
 * (e.g. `authorizenter-core.zip`) that extracts to a single `<slug>/` folder. The
...
class Github_Updater {
	...
	/**
	 * Plugin slug / folder name (e.g. "authorizenter-core").
	 *
	 * @var string
	 */
	private $slug;
	...
	/**
	 * Plugin basename, e.g. "authorizenter-core/authorizenter-core.php".
	 *
	 * @return string
	 */
	...
	private function latest_release() {
		$cache_key = 'authorizenter_gh_' . md5( $this->repo );
		...
		$headers = array(
			...
			'User-Agent' => 'Authorizenter-Updater',
		);
		 ...
		 */
		$args = apply_filters(
			'authorizenter_github_request_args',
			array(
				'timeout' => 15,
				'headers' => $headers,
			),
