...
class Questions {
	...
	const ANSWERS_META = 'authorizenter_answers';

	/** Prefix for per-question mirror meta keys (indexed, fast to query). */
	const ANSWER_META_PREFIX = 'authorizenter_answer_';
	...
	public function all() {
		 ...
		 */
		return apply_filters( 'authorizenter_questions', $list );
	}
	...
	public function save_answers( $user_id, array $input ) {
		...
		foreach ( $questions as $id => $q ) {
			...
			if ( ! empty( $q['required'] ) && $this->is_empty_answer( $clean ) && ! array_key_exists( $id, $stored ) ) {
				return new \WP_Error(
					'authorizenter_answer_required',
					/* translators: %s: question label */
					sprintf( __( 'Please answer: %s', 'authorizenter' ), $q['label'] ),
					array(
						'status'   => 400,
						'question' => $id,
					)
		 ...
		 */
		do_action( 'authorizenter_answers_saved', $user_id, $stored );
		...
		if ( ! $this->has_pending_required( $user_id ) ) {
			 ...
			 * @param int $user_id User id.
			 */
			do_action( 'authorizenter_questions_completed', $user_id );
		}
