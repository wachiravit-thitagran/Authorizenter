...
class JWT_Verifier {
	...
	public function verify( $id_token, $jwks_uri, $issuer, $audience, $nonce = '' ) {
		if ( ! class_exists( JWT::class ) || ! class_exists( JWK::class ) ) {
			return new \WP_Error(
				'authorizenter_jwt_missing',
				__( 'The firebase/php-jwt library is required for OIDC verification. Run composer install.', 'authorizenter' ),
				array( 'status' => 500 )
			);
		}
		...
		if ( '' !== $issuer && ( ! isset( $decoded['iss'] ) || $decoded['iss'] !== $issuer ) ) {
			return new \WP_Error( 'authorizenter_jwt_iss', __( 'ID token issuer mismatch.', 'authorizenter' ), array( 'status' => 401 ) );
		}

		$aud = isset( $decoded['aud'] ) ? (array) $decoded['aud'] : array();
		if ( '' !== $audience && ! in_array( $audience, $aud, true ) ) {
			return new \WP_Error( 'authorizenter_jwt_aud', __( 'ID token audience mismatch.', 'authorizenter' ), array( 'status' => 401 ) );
		}

		if ( '' !== $nonce && ( ! isset( $decoded['nonce'] ) || ! hash_equals( $nonce, (string) $decoded['nonce'] ) ) ) {
			return new \WP_Error( 'authorizenter_jwt_nonce', __( 'ID token nonce mismatch.', 'authorizenter' ), array( 'status' => 401 ) );
		}
	...
	private function fetch_jwks( $jwks_uri ) {
		...
		if ( ! is_array( $data ) || empty( $data['keys'] ) ) {
			return new \WP_Error( 'authorizenter_jwks_error', __( 'Could not fetch provider signing keys.', 'authorizenter' ), array( 'status' => 502 ) );
		}
