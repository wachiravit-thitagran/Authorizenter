...
abstract class Provider_Base {
	...
	protected function request_token( array $body ) {
		...
		if ( $code < 200 || $code >= 300 || ! is_array( $data ) ) {
			return new \WP_Error(
				...
				sprintf( __( 'Token exchange failed for provider %s.', 'authorizenter' ), $this->id() ),
				array( 'status' => 502 )
			);
		}
	...
	protected function request_userinfo( $url, $access_token ) {
		...
		$data = json_decode( wp_remote_retrieve_body( $response ), true );
		if ( ! is_array( $data ) ) {
			return new \WP_Error( 'authorizenter_userinfo_error', __( 'Could not read user profile.', 'authorizenter' ), array( 'status' => 502 ) );
		}
		return $data;
	}
}
