 ...
 *
 * The login page itself and anything allowed via the `authorizenter_private_allow`
...
class Private_Site {
	...
	public function maybe_redirect() {
		...
		$login_page_id = (int) apply_filters( 'authorizenter_login_page_id', 0 );
		 ...
		 */
		$is_allowed = (bool) apply_filters( 'authorizenter_private_allow', false );

		if ( ! $this->should_block( false, $is_login_page, $is_allowed ) ) {
			return;
		}

		$login = apply_filters( 'authorizenter_login_url', wp_login_url( home_url( add_query_arg( array() ) ) ) );
