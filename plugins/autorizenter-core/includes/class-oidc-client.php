...
class Oidc_Client {
	...
	public function start( array $config, $provider_url, $redirect_uri, array $scopes ) {
		...
		try {
			...
			return new \WP_Error( 'autorizenter_oidc_start_failed', $e->getMessage(), array( 'status' => 502 ) );
		}

		// authenticate() redirects + exits on the no-code leg; reaching here is unexpected.
		return new \WP_Error( 'autorizenter_oidc_no_redirect', __( 'Could not start the OIDC sign-in.', 'autorizenter' ), array( 'status' => 500 ) );
	}
	...
	public function complete( array $config, $provider_url, $redirect_uri, array $scopes ) {
		...
		try {
			...
			if ( ! $ok ) {
				return new \WP_Error( 'autorizenter_oidc_auth_failed', __( 'OIDC authentication failed.', 'autorizenter' ), array( 'status' => 401 ) );
			}
			...
			try {
				...
				autorizenter_log( 'oidc userinfo unavailable', array( 'message' => $e->getMessage() ) );
			}

			return $claims;
		} catch ( \Throwable $e ) {
			return new \WP_Error( 'autorizenter_oidc_error', $e->getMessage(), array( 'status' => 502 ) );
		}
	}
	...
	private function client( array $config, $provider_url, $redirect_uri, array $scopes ) {
		if ( ! $this->is_available() ) {
			return new \WP_Error(
				'autorizenter_oidc_lib_missing',
				__( 'The jumbojett/openid-connect-php library is required for OIDC sign-in. Run composer install.', 'autorizenter' ),
				array( 'status' => 500 )
			);
		}
		if ( '' === (string) $provider_url ) {
			return new \WP_Error( 'autorizenter_oidc_no_provider_url', __( 'No OIDC issuer/discovery URL configured.', 'autorizenter' ), array( 'status' => 500 ) );
		}
		 ...
		 * @param array                          $config Provider config.
		 */
		return apply_filters( 'autorizenter_oidc_client', $client, $config );
	}
}
